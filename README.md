...
## 🌟 A História por Trás do AuraVMS
...
É exatamente para responder a essa pergunta — de forma rápida, organizada e eficiente — que o **AuraVMS** foi criado.

---

## 🔍 O que é o AuraVMS?

O **AuraVMS** (Aura Video Management System) é uma **plataforma de gerenciamento de câmeras de segurança** desenvolvida em Python com Flask, que transforma câmeras IP comuns espalhadas por um bairro em uma **rede colaborativa de vigilância comunitária**.
...
> 💡 **A grande sacada:** em vez de cada câmera ficar isolada no seu próprio DVR, o AuraVMS conecta todas elas numa rede colaborativa, onde o acesso é controlado por permissões granulares e grupos. É o poder da comunidade, organizado digitalmente.
...
**Sem o AuraVMS:** cada câmera estaria num sistema diferente, exigiria senha individual, e a coordenação levaria horas — tempo que pode ser crítico.
...
3. **Relay via go2rtc:** ao abrir o feed, o AuraVMS instrui o go2rtc a se conectar à câmera via RTSP e re-transmitir o sinal como WebRTC
...
```bash
git clone https://github.com/rfernandoufabc/aura-vms.git
cd aura-vms
```
...
[Servidor na Nuvem ou VPS]
    ...
    │       ├── / → AuraVMS (Gunicorn, porta 5000)
    │       └── video.seudominio.com → go2rtc (porta 1984)
    └── go2rtc (com acesso VPN/tunnel às câmeras locais)
```

O AuraVMS detecta automaticamente se está rodando em localhost/IP local ou num domínio e ajusta a URL do go2rtc de acordo — sem configuração manual adicional.
...
| Aspecto | Solução Tradicional | AuraVMS |
|---|---|---|
